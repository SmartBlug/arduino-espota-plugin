      ...
      public void run() {
        try {
          if(listenOnProcess(arguments) != 0){
            ...
            editor.statusNotice("Image Uploaded");
          }
        } catch (Exception e){
          editor.statusError("Upload failed!");
        }
...
*/
  public void run() {
    ...
    TargetPlatform platform = BaseNoGui.getTargetPlatform();
    String espotaCmd = "espota.py";
    File espota = new File(platform.getFolder()+"/tools",espotaCmd);
    String sketchName = editor.getSketch().getName();
    String imagePath = getBuildFolderPath(editor.getSketch()) + "/" + sketchName + ".ino.bin";
 
    //System.out.println(editor.getSketch().getBuildPath().getAbsolutePath());

    final String[] dstIP = {""};
    //String srcIP = "192.168.5.100";

    String config = ""; 
    File f = new File(imagePath);
    if(f.exists() && !f.isDirectory()) { 
      try (Stream<String> lines = Files.lines(Paths.get(editor.getSketch().getFolder()+"/ESPOTA.h"))) {
        lines.forEach( line -> {
      ...
      //System.out.println(dstIP[0]);
      //config = new String(Files.readAllBytes(Paths.get(editor.getSketch().getFolder()+"/ESPOTA.h")));

      //File esptool = new File(platform.getFolder()+"/tools");
      //String toolPath = esptool.getAbsolutePath();
       //    createAndUpload();
      //System.out.println(espota.getAbsolutePath());
      //System.out.println(toolPath);
      //System.out.println(sketchName);
      //System.out.println(imagePath);

      //System.out.println(editor.getSketch().getFolder());
      //System.out.println(config);

      if (dstIP[0]!="") {
        System.out.println("Sending "+sketchName+".ino.bin to "+dstIP[0]+"...");
        sysExec(new String[]{pythonCmd, espota.getAbsolutePath(), "-P", "5353", "-i", dstIP[0], "-f", imagePath});
        //System.out.println("Done !");
      }
      else System.out.println("You need to specify IP in ESPOTA.h like his : // IP=1.2.3.4");
    }
    else {
      System.out.println("You need to compile first");
    }
  }
}
